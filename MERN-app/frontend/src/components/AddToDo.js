...
function AddToDo({ setToDoListItems }) {
  ...
  const handleAddToDo = async (e) => {
    ...
    //The email and jwt token being parsed from the localstorage to be used in the fetch
    const email = JSON.parse(localStorage.getItem("user"));
    const token = JSON.parse(localStorage.getItem("token"));

    //The try catch communicates with the backend server
    //Uses the post method to add an item to the users to do list
    try {
